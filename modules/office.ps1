...
function Get-TNOfficeDeploymentTool {
    ...
    if (Get-Command Write-TNLog -ErrorAction SilentlyContinue) { Write-TNLog 'Downloading Microsoft Office Deployment Tool.' }
    ...
    try {
        ...
        if (-not $match.Success) { throw 'Could not resolve the current Office Deployment Tool download URL from Microsoft.' }
        ...
        Remove-Item $odtExe -Force -ErrorAction SilentlyContinue
        if (-not (Test-Path $setupExe)) { throw 'ODT extraction completed but setup.exe was not found.' }
        return $setupExe
    } catch {
        if (Get-Command Write-TNLog -ErrorAction SilentlyContinue) { Write-TNLog "Office Deployment Tool download failed: $($_.Exception.Message)" }
        throw
    }
}

function Get-TNOfficePreset {
    param([Parameter(Mandatory)][ValidateSet('M365Enterprise','LTSC2024ProPlus','LTSC2024Standard','LTSC2021ProPlus','LTSC2021Standard')][string]$Edition)
    switch ($Edition) {
        'M365Enterprise' { [pscustomobject]@{ ProductId='O365ProPlusRetail'; Channel='MonthlyEnterprise' } }
        'LTSC2024ProPlus' { [pscustomobject]@{ ProductId='ProPlus2024Volume'; Channel='PerpetualVL2024' } }
        'LTSC2024Standard' { [pscustomobject]@{ ProductId='Standard2024Volume'; Channel='PerpetualVL2024' } }
        'LTSC2021ProPlus' { [pscustomobject]@{ ProductId='ProPlus2021Volume'; Channel='PerpetualVL2021' } }
        'LTSC2021Standard' { [pscustomobject]@{ ProductId='Standard2021Volume'; Channel='PerpetualVL2021' } }
    }
}
...
function New-TNOfficeConfiguration {
    [CmdletBinding()]
    param(
        [ValidateSet('M365Enterprise','LTSC2024ProPlus','LTSC2024Standard','LTSC2021ProPlus','LTSC2021Standard')][string]$Edition='M365Enterprise',
        [ValidateSet('64','32')][string]$Architecture='64',
        [ValidatePattern('^[a-z]{2}-[a-z]{2}$')][string]$Language='en-us',
        [ValidateSet('Current','MonthlyEnterprise','SemiAnnual')][string]$M365Channel='MonthlyEnterprise',
        [string[]]$ExcludeApps=@(), [switch]$RemoveMSI, [string]$Path
    )
    $preset = Get-TNOfficePreset -Edition $Edition
    $channel = if ($Edition -eq 'M365Enterprise') { $M365Channel } else { $preset.Channel }
    if (-not $Path) { $Path = Join-Path (Get-TNOfficeWorkDir) 'configuration.xml' }
    $validApps = @('Access','Excel','Groove','Lync','OneDrive','OneNote','Outlook','PowerPoint','Publisher','Teams','Word')
    $excludeNodes = foreach ($app in ($ExcludeApps | Select-Object -Unique)) { if ($validApps -contains $app) { "      <ExcludeApp ID=`"$app`" />" } }
    $removeMsiNode = if ($RemoveMSI) { '  <RemoveMSI />' } else { $null }
    $xml = @('<Configuration>',"  <Add OfficeClientEdition=`"$Architecture`" Channel=`"$channel`">","    <Product ID=`"$($preset.ProductId)`">","      <Language ID=`"$Language`" />",$excludeNodes,'    </Product>','  </Add>','  <Updates Enabled="TRUE" />','  <Display Level="Full" AcceptEULA="TRUE" />',$removeMsiNode,'</Configuration>') | Where-Object { $_ -ne $null }
    Set-Content -Path $Path -Value ($xml -join "`r`n") -Encoding UTF8 -Force
    if (Get-Command Write-TNLog -ErrorAction SilentlyContinue) { Write-TNLog "Office configuration generated: Edition=$Edition Architecture=$Architecture Language=$Language Channel=$channel" }
    return $Path
}
...
function Install-TNOffice {
    ...
    param(
        [ValidateSet('M365Enterprise','LTSC2024ProPlus','LTSC2024Standard','LTSC2021ProPlus','LTSC2021Standard')][string]$Edition='M365Enterprise',
        [ValidateSet('64','32')][string]$Architecture='64', [string]$Language='en-us',
        [ValidateSet('Current','MonthlyEnterprise','SemiAnnual')][string]$M365Channel='MonthlyEnterprise', [string[]]$ExcludeApps=@(), [switch]$RemoveMSI
    )
...
function Download-TNOfficeOffline {
    ...
    param(
        [ValidateSet('M365Enterprise','LTSC2024ProPlus','LTSC2024Standard','LTSC2021ProPlus','LTSC2021Standard')][string]$Edition='M365Enterprise',
        [ValidateSet('64','32')][string]$Architecture='64', [string]$Language='en-us',
        [ValidateSet('Current','MonthlyEnterprise','SemiAnnual')][string]$M365Channel='MonthlyEnterprise', [string[]]$ExcludeApps=@()
    )
    $setupExe = Get-TNOfficeDeploymentTool
    ...
    if ($process.ExitCode -ne 0) { throw "Office download failed with exit code $($process.ExitCode)." }
...
function Get-TNOfficeStatus {
    ...
    foreach ($path in @('HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Office\ClickToRun\Configuration')) {
        if (Test-Path $path) {
            ...
            return [pscustomobject]@{ Installed=$true; ProductReleaseIds=$cfg.ProductReleaseIds; Version=$cfg.VersionToReport; Architecture=$cfg.Platform; UpdateChannel=$cfg.UpdateChannel }
        }
    }
    [pscustomobject]@{ Installed=$false; ProductReleaseIds=$null; Version=$null; Architecture=$null; UpdateChannel=$null }
}
