function Show-TNUtilityDashboard {
    ...
    $categoryPanel.BackColor = $bgColor
    ...
    $leftPanel.BackColor = $panelColor
    ...
    foreach ($cat in $categories) {
        $button = New-TNButton -Caption $cat -X 0 -Y 0 -W 132 -H 30
        $button.Tag = $cat
        $button.Add_Click({
    ...
    $presetTitle.ForeColor = $cyanColor
    ...
    $btnStandard = New-TNButton -Caption 'Standard' -X 20 -Y 52 -W 170 -H 34
    $btnAdvanced = New-TNButton -Caption 'Advanced' -X 200 -Y 52 -W 170 -H 34
    $btnClearTweaks = New-TNButton -Caption 'Clear' -X 380 -Y 52 -W 130 -H 34
    ...
    $tweakLabels = @(
        ...
        'Disable background app access for current user',
    ...
    $selectPreset = {
        ...
        for ($i=0;$i -lt $tweakBox.Items.Count;$i++) {
            $label = [string]$tweakBox.Items[$i]
            $enable = $Preset -eq 'Advanced' -or ($label -notmatch 'background|Ultimate')
            if ($enable) { $tweakBox.SetItemChecked($i,$true) }
        }
    }
    ...
    $btnRunTweaks = New-TNButton -Caption 'Run Selected Tweaks' -X 20 -Y 575 -W 190 -H 38
    $btnRunTweaks.BackColor = $accentColor
    ...
    $lblEdition.ForeColor = $cyanColor
    ...
    $officeApps.Size = New-Object System.Drawing.Size(560,300)
    $officeApps.CheckOnClick = $true
    Set-DarkControl $officeApps
    foreach ($appName in @('Access','Excel','OneDrive','OneNote','Outlook','PowerPoint','Publisher','Teams','Word')) {
        [void]$officeApps.Items.Add($appName,$false)
    }
    $tabOffice.Controls.Add($officeApps)

    $officeHint = New-Object System.Windows.Forms.Label
    $officeHint.Text = 'Checked apps will be EXCLUDED from the Office installation.'
    $officeHint.ForeColor = $mutedColor
    $officeHint.AutoSize = $true
    $officeHint.Location = New-Object System.Drawing.Point(25,475)
    $tabOffice.Controls.Add($officeHint)

    $getExcluded = {
        @($officeApps.CheckedItems | ForEach-Object { [string]$_ })
    }

    $btnOfficeInstall = New-TNButton -Caption 'Install Office' -X 25 -Y 520 -W 170 -H 38
    $btnOfficeInstall.BackColor = $accentColor
    $btnOfficeDownload = New-TNButton -Caption 'Download Offline' -X 210 -Y 520 -W 180 -H 38
    $btnOfficeXml = New-TNButton -Caption 'Generate XML' -X 405 -Y 520 -W 160 -H 38
    $tabOffice.Controls.AddRange(@($btnOfficeInstall,$btnOfficeDownload,$btnOfficeXml))

    $btnOfficeInstall.Add_Click({
        try {
            Install-TNOffice -Edition ([string]$edition.SelectedItem) -Architecture ([string]$arch.SelectedItem) -Language $lang.Text -ExcludeApps (& $getExcluded) -Confirm:$false
        } catch {
            [System.Windows.Forms.MessageBox]::Show($_.Exception.Message,'Office Deployment Error') | Out-Null
        }
    })

    $btnOfficeDownload.Add_Click({
        try {
            Download-TNOfficeOffline -Edition ([string]$edition.SelectedItem) -Architecture ([string]$arch.SelectedItem) -Language $lang.Text -ExcludeApps (& $getExcluded)
        } catch {
    ...
    $btnOfficeXml.Add_Click({
        try {
            $path = New-TNOfficeConfiguration -Edition ([string]$edition.SelectedItem) -Architecture ([string]$arch.SelectedItem) -Language $lang.Text -ExcludeApps (& $getExcluded)
            [System.Windows.Forms.MessageBox]::Show("Configuration saved to:`r`n$path",'Terra Nova IT Utility') | Out-Null
        } catch {
            [System.Windows.Forms.MessageBox]::Show($_.Exception.Message,'Office XML Error') | Out-Null
        }
    })
