...
# Uses Winget when available, with Chocolatey fallback.
...
function Get-TNAppCatalog {
    [CmdletBinding()] param([string]$Category)
    if ([string]::IsNullOrWhiteSpace($Category) -or $Category -eq 'All') { return $script:TNAppCatalog }
    $script:TNAppCatalog | Where-Object Category -eq $Category
}

function Test-TNWingetAvailable { return [bool](Get-Command winget.exe -ErrorAction SilentlyContinue) }

function Install-TNSelectedApps {
    [CmdletBinding()]
    param([Parameter(Mandatory)][string[]]$Names,[ValidateSet('Auto','Winget','Chocolatey')][string]$PackageManager='Auto')

    $results = New-Object System.Collections.Generic.List[object]
    foreach ($name in $Names) {
        $app = $script:TNAppCatalog | Where-Object Name -eq $name | Select-Object -First 1
        if (-not $app) { $results.Add([pscustomobject]@{Name=$name;Success=$false;Source='None';Message='Unknown application'}); continue }
        ...
        try {
            if ($useWinget -and $app.WingetId) {
                ...
                if ($wingetExit -in @(0,-1978335189)) {
                    ...
                    $results.Add([pscustomobject]@{Name=$app.Name;Success=$true;Source='Winget';Message='Installation completed'}); $completed=$true
                } elseif ($PackageManager -ne 'Auto' -or -not $app.Choco) { throw "Winget exit code: $wingetExit" }
            }
            if (-not $completed -and $app.Choco) {
                if (-not (Test-Path 'C:\ProgramData\chocolatey\bin\choco.exe') -and (Get-Command Install-ChocolateyIfMissing -ErrorAction SilentlyContinue)) { Install-ChocolateyIfMissing | Out-Null }
                ...
                $results.Add([pscustomobject]@{Name=$app.Name;Success=$true;Source='Chocolatey';Message='Installation completed'}); $completed=$true
            }
            if (-not $completed) { throw 'No supported package source is configured for this app.' }
        } catch {
    ...
    $successes=@($results|Where-Object Success); $failures=@($results|Where-Object{-not $_.Success})
    Write-Host ''; Write-Host '========== Installation Summary ==========' -ForegroundColor Cyan
    foreach($result in $results){ if($result.Success){Write-Host "[OK] $($result.Name) - $($result.Source)" -ForegroundColor Green}else{Write-Host "[FAILED] $($result.Name) - $($result.Message)" -ForegroundColor Red} }
    ...
    if ('System.Windows.Forms.MessageBox' -as [type]) {
        ...
        if($successes.Count -gt 0){$message+="`r`n`r`nInstalled:`r`n- "+(($successes|ForEach-Object Name)-join "`r`n- ")}
        if($failures.Count -gt 0){$message+="`r`n`r`nFailed:`r`n- "+(($failures|ForEach-Object Name)-join "`r`n- ")}
        $icon=if($failures.Count -eq 0){[System.Windows.Forms.MessageBoxIcon]::Information}else{[System.Windows.Forms.MessageBoxIcon]::Warning}
        [System.Windows.Forms.MessageBox]::Show($message,'Terra Nova IT Utility - Installation Complete',[System.Windows.Forms.MessageBoxButtons]::OK,$icon)|Out-Null
    }
    return $results
}
